...
//手势长按事件
- (void)longPressActionInImageView:(UILongPressGestureRecognizer *)longPress {
    
    if (_delegate && [_delegate respondsToSelector:@selector(carouselView:longPressImageAtIndex:image:)]) {
        // 如果_nextPhotoIndex == 0,那么中间那张图片一定是数组中最后一张，我们要传的就是中间那张图片在数组中的下标
        if (_nextPhotoIndex == 0) {
            
            [_delegate carouselView:self longPressImageAtIndex:_kImageCount -1 image:self.currentImgView.image];
        }else{
            [_delegate carouselView:self longPressImageAtIndex:_nextPhotoIndex-1 image:self.currentImgView.image];
        }
    }
}
...
-(UIImageView *)currentImgView{
    if (_currentImgView == nil) {
        ...
        UILongPressGestureRecognizer * longPress = [[UILongPressGestureRecognizer alloc]initWithTarget:self action:@selector(longPressActionInImageView:)];
        
        
        //长按时候,手指头可以移动的距离
//    longPress.allowableMovement = 30;
        
        [_currentImgView addGestureRecognizer:longPress];
...
@implementation LXMSPCarouselView
...
- (void)layoutSubviews {
    ...
    self.lastImgView.frame = CGRectMake(30, 6, kWidth-60, kHeight-12);
    self.currentImgView.frame = CGRectMake(kWidth+30, 6, kWidth-60, kHeight-12);
    self.nextImgView.frame = CGRectMake(kWidth * 2 + (30), 6, kWidth-60, kHeight-12);
...
-(void)scrollViewDidScroll:(UIScrollView *)scrollView{
    ...
    // 到第一张图片时   (一上来，当前图片的x值是kWidth)
    if (ceil(scrollView.contentOffset.x) <= 0) {  // 右滑
        self.nextImgView.image = self.currentImgView.image;
        self.currentImgView.image = self.lastImgView.image;
        // 将轮播图的偏移量设回中间位置
        scrollView.contentOffset = CGPointMake(kWidth, 0);
        self.lastImgView.image = nil;
        // 一定要是小于等于，否则数组中只有一张图片时会出错
        if (self.lastPhotoIndex <= 0) {
            self.lastPhotoIndex = self.kImageCount - 1;
            self.nextPhotoIndex = self.lastPhotoIndex - (self.kImageCount - 2);
        } else {
            self.lastPhotoIndex--;
            if (self.nextPhotoIndex == 0) {
                self.nextPhotoIndex = self.kImageCount - 1;
            } else {
                self.nextPhotoIndex--;
            }
        }
        [self setImageView:self.lastImgView withSubscript:self.lastPhotoIndex];
    }
    // 到最后一张图片时（最后一张就是轮播图的第三张）
    if (ceil(scrollView.contentOffset.x)  >= kWidth*2) {  // 左滑
        self.lastImgView.image = self.currentImgView.image;
        self.currentImgView.image = self.nextImgView.image;
        // 将轮播图的偏移量设回中间位置
        scrollView.contentOffset = CGPointMake(kWidth, 0);
        self.nextImgView.image = nil;
        // 一定要是大于等于，否则数组中只有一张图片时会出错
        if (self.nextPhotoIndex >= self.kImageCount - 1 ) {
            self.nextPhotoIndex = 0;
            self.lastPhotoIndex = self.nextPhotoIndex + (self.kImageCount - 2);
        } else{
            self.nextPhotoIndex++;
            if (self.lastPhotoIndex == self.kImageCount - 1) {
                self.lastPhotoIndex = 0;
            } else {
                self.lastPhotoIndex++;
            }
        }
        [self setImageView:self.nextImgView withSubscript:self.nextPhotoIndex];
    }
    
    if (self.nextPhotoIndex - 1 < 0) {
        self.pageControl.currentPage = self.kImageCount - 1;
    } else {
        self.pageControl.currentPage = self.nextPhotoIndex - 1;
    }
    
    
    if (self.nextPhotoIndex == 0) {
        self.picIndex = self.kImageCount - 1;
    }else{
        self.picIndex = self.nextPhotoIndex - 1;
    }
